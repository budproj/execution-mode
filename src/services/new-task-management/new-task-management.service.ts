...
export class NewTaskManagementService {
  ...
  async getAllTasks(parameters: ParsedUrlQuery) {
    ...
    */
    ...
    const { data: response } = await this.client.get<Task[]>(`task-management/task/list`, {
      params: parameters,
    })
    return response
  }
  ...
  async getTask(id: Task['id']) {
    const { data: response } = await this.client.get<Task>(`task-management/task/get/${id}`)

    return response
  }

  async addTask(data: TaskInsert) {
    const { data: response } = await this.client.post<NewTask>(`task-management/task/create`, data)
    return response
  }

  async removeTask(task_id: string) {
    // Tasks are soft deleted
    await this.client.delete<NewTask>(`task-management/task/delete/${task_id}`)
  }
  ...
  async updateTask(taskId: string, data: Partial<Task>) {
    ...
    const { data: response } = await this.client.put<Task>(
      `task-management/task/update/${taskId}`,
      data,
    )
